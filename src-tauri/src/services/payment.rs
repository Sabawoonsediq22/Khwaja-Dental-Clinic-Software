...
impl PaymentService {
    pub async fn add(pool: &SqlitePool, input: AddPaymentInput) -> AppResult<Payment> {
        ...
        let payment = sqlx::query_as::<_, Payment>(
            "INSERT INTO payments (id, invoice_id, amount_afn, amount_usd, method, notes, received_at)
             VALUES (?, ?, ?, ?, ?, ?, ?)
             RETURNING id, invoice_id, amount_afn, amount_usd, method, notes, received_at"
        )
        .bind(&id)
    ...
    pub async fn list_for_invoice(pool: &SqlitePool, invoice_id: &str) -> AppResult<Vec<Payment>> {
        let payments = sqlx::query_as(
            "SELECT id, invoice_id, amount_afn, amount_usd, method, notes, received_at FROM payments WHERE invoice_id = ?"
        )
        .bind(invoice_id)
        .fetch_all(pool)
        .await?;
